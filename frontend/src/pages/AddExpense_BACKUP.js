...
export default function AddExpense() {
  ...
  return (
    <div id="add-expense-page" style={{ fontFamily: 'Poppins, Inter, sans-serif', minHeight: '100vh', background: 'linear-gradient(135deg, #e0f7fa 0%, #e8f5e9 100%)', display: 'flex', flexDirection: 'column', paddingBottom: '80px' }}>
      {/* Google Fonts */}
      ...
      <div style={{ padding: '1.5rem 1.25rem', maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ 
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            {/* Name field (hidden but prefilled) */}
            <input type="hidden" name="user_name" value={form.user_name} />
            <input type="hidden" name="email" value={form.email} />            <form onSubmit={handleSubmit}>
              
              {/* User Info Section */}
              <div className="ae-two" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '0.6rem', fontWeight: 600, color: '#333', fontSize: window.innerWidth <= 480 ? '0.9rem' : '1rem' }}>Name</label>
                  <input
                    type="text"
                    name="user_name"
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '0.6rem', fontWeight: 600, color: '#333', fontSize: window.innerWidth <= 480 ? '0.9rem' : '1rem' }}>Email</label>
                  <input
                    type="email"
                    name="email"
                    value={form.email}
              ...
              <div className="ae-two" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '0.6rem', fontWeight: 600, color: '#333', fontSize: window.innerWidth <= 480 ? '0.9rem' : '1rem' }}>Category</label>
                  <select
                    name="category"
                    value={form.category}
                    onChange={handleChange}
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '0.6rem', fontWeight: 600, color: '#333', fontSize: window.innerWidth <= 480 ? '0.9rem' : '1rem' }}>Amount (₹)</label>
                  <input
                    type="number"
                    name="amount"
                    value={form.amount}
              ...
              <div className="ae-three" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '2rem', marginBottom: '2.5rem' }}>
                <div>
      ...
      {/* Mobile-responsive alignment tweaks (no layout changes, only stacking on small screens) */}
      <style>{`
        @media (max-width: 1024px) {
          #add-expense-page .ae-content { padding: 2rem 1.25rem; }
        }
        @media (max-width: 768px) {
          #add-expense-page .ae-content { padding: 1rem !important; }
          #add-expense-page .ae-grid { grid-template-columns: 1fr !important; gap: 1.5rem !important; }
          #add-expense-page .ae-card { padding: 1rem !important; }
          #add-expense-page .ae-two { grid-template-columns: 1fr !important; gap: 1rem !important; margin-bottom: 1.5rem !important; }
          #add-expense-page .ae-three { grid-template-columns: 1fr !important; gap: 1rem !important; }
          #add-expense-page footer { padding: 2rem 1rem !important; margin-top: 2rem !important; }
          #add-expense-page footer > div { flex-direction: column !important; gap: 1rem !important; }
        }
        @media (max-width: 480px) {
          #add-expense-page .ae-content { padding: 0.75rem !important; }
          #add-expense-page .ae-card { padding: 0.875rem !important; border-radius: 16px !important; }
          #add-expense-page h1 { font-size: 1.6rem !important; }
          #add-expense-page select, 
          #add-expense-page input[type="text"],
          #add-expense-page input[type="email"],
          #add-expense-page input[type="number"],
          #add-expense-page input[type="date"],
          #add-expense-page input[type="time"],
        ...
        @supports (padding: max(0px)) {
          #add-expense-page { padding-bottom: max(0px, env(safe-area-inset-bottom)); }
        }
      `}</style>
    </div>
  );
